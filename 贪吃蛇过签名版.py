...
class TCS:
    ...
    def req1(self, api, data, uid, sid, wua='False', v="1.0"):
        try:
            if type(data) == dict:
    ...
    def getOpenId(self):
        ...
        try:
            res1 = self.req(api1, data1, "1.0")
            if res1.json()['ret'][0] == 'SUCCESS::调用成功':
                nested_data = json.loads(res1.json()["data"]["data"])
                self.openId = nested_data['openId']
                return True
            else:
    ...
    def startgame(self):
        ...
        try:
            ...
            if res is None:
                return None
            if res.json()["ret"][0] == "SUCCESS::调用成功":
                resJson = json.loads(res.json()['data']['data']).get('players')
                self.players = resJson
                pattern = r'"gameId":"(\w+)"'
                pattern2 = r'"gameToken":"(\w+)"'
                match = re.search(pattern, str(res.json()))
                match2 = re.search(pattern2, str(res.json()))
                if match:
                    self.gameId = match.group(1)
                    self.gameToken = match2.group(1)
                    return True
                else:
                    print("❎解析数据失败")
                    return None
