...
def get_ck_usid(ck1):
    ...
    for pair in key_value_pairs:
        pair = pair.strip()  # 去掉首尾空格，确保键值对格式正确
        if "=" in pair:
            key, value = pair.split("=", 1)  # 使用 maxsplit=1 避免过多等号问题
            if key == "USERID":
                return value
    return '账号'  # 如果遍历所有键值对后找不到 USERID，返回默认值
...
class TYT:
    ...
    def xsign(self, api, data, wua, v):
        ...
        try:
            r = requests.post(
                "http://10.0.8.4:9999/api/getXSign",
                json=body
            )
            r.raise_for_status()
            return r.json()
    ...
    def login(self):
        ...
        data1 = {}
        try:
            res1 = self.xsign_req(api1, data1, "1.0")
            ...
            if res.json()['ret'][0] == 'SUCCESS::接口调用成功':
                if res.json()["data"]["mlist"][0]["stage"]['count'] < 5:
                    ...
                    for _ in range(int(count)):
                        ...
                        try:
                            res1 = self.xsign_req(api, data, "1.0")
                            if res1:
                                if '异常' not in res1:
